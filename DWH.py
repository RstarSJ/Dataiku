...
def is_active(df=None, logger=None):
    """
        This function sets is_active column
        :param df: data frame
        :param logger: logger
        :return: returns data with is_active data
        """
    try:
        logger.info("Inside DataDWH : is_active()")

        if df is None:
            raise Exception("data frame is none")
        # df.insert(10, 'is_active', 'Y')
        df['IS_ACTIVE'] = 'Y'

        return df

    except Exception as e2:
        error = "Failed to derive is_active  {}".format(str(e2))
        logger.error(error)
...
if __name__ == "__main__":
    try:                
        ...
        integration_space_eed23760_dku_node_b3d4291b_NVS_df = integration_space_eed23760_dku_node_b3d4291b_NVS.get_dataframe()
        ...
        is_active_obj = is_active(df=integration_space_eed23760_dku_node_b3d4291b_NVS_df, logger=logger_obj)

        dataware_HOUSE1_space_eed23760_dku_node_b3d4291b_NVS_df = is_active_obj # For this sample code, simply copy input to output


        # Write recipe outputs
        dataware_HOUSE1_space_eed23760_dku_node_b3d4291b_NVS = dataiku.Dataset("DATAWARE_HOUSE1_space-eed23760-dku_node-b3d4291b_NVS")
        dataware_HOUSE1_space_eed23760_dku_node_b3d4291b_NVS.write_with_schema(dataware_HOUSE1_space_eed23760_dku_node_b3d4291b_NVS_df)
